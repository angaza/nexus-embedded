 ...
 *
 * A device may be assigned any value of PAYG credit from 0 to (UINT32_MAX - 1),
 * but (UINT32_MAX) represents the special 'device is unlocked' (PAYG
 * unrestricted) case.
 */
#define NXP_CHANNEL_PAYG_CREDIT_REMAINING_UNLOCKED_SENTINEL_VALUE UINT32_MAX
...
void nexus_channel_res_payg_credit_init(void)
{
    ...
    const nx_channel_error result =
        ...
        nx_channel_register_resource("/c",
                                     ...
                                     if_mask_arr,
                                     OC_POST,
                                     nexus_channel_res_payg_credit_post_handler,
                                     true); // secured
#ifdef NEXUS_DEFINED_DURING_TESTING
