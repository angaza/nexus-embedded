...
#if NEXUS_CHANNEL_LINK_SECURITY_ENABLED
    // determine if the requested resource method is secured
    oc_resource_t* res = oc_ri_get_app_resource_by_uri(
      oc_string(client_cb->uri), oc_string_len(client_cb->uri), NEXUS_CHANNEL_NEXUS_DEVICE_ID);

    secured = nexus_channel_sm_resource_method_is_secured(res, client_cb->method);

    // get Nexus ID
    struct nx_id nexus_id = {0};
    (void) nexus_oc_wrapper_oc_endpoint_to_nx_id(&client_cb->endpoint, &nexus_id);
