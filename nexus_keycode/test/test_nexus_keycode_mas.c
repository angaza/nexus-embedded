...
static void _push_key_sequence(const char* key_chars)
{
    for (uint16_t i = 0; key_chars[i] != '\0'; ++i)
    {
        ...
        // port_feedback_start_ExpectAnyArgs();
    }
...
static void _mas_bookend_push_chars_check_feedback(
    ...
    const enum port_feedback_type scripts[32],
    bool prevent_rate_limit)
{
    for (uint16_t i = 0; key_chars[i] != '\0'; ++i)
    {
        ...
        port_feedback_start_ExpectAndReturn(scripts[i], true);
...
static void _mas_bookend_push_chars_no_check_feedback(const char* key_chars)
{
    for (uint16_t i = 0; key_chars[i] != '\0'; ++i)
    {
        port_request_processing_Ignore();
        port_feedback_start_IgnoreAndReturn(true);
    ...
    void)
{
    struct test_scenario
    {
        ...
        enum port_feedback_type expected_scripts[32];
        unsigned int expected_scripts_started;
        struct nexus_keycode_frame expected_frames[32];
        unsigned int expected_frame_count;
    };

    struct test_scenario scenarios[] = {// repeated start keys
                                        {"***",
                                         {
                                             PORT_FEEDBACK_TYPE_KEY_ACCEPTED,
                                             PORT_FEEDBACK_TYPE_KEY_ACCEPTED,
                                             PORT_FEEDBACK_TYPE_KEY_ACCEPTED,
                                         },
                                         3,
                                        ...
                                        {"333",
                                         {
                                             PORT_FEEDBACK_TYPE_KEY_REJECTED,
                                             PORT_FEEDBACK_TYPE_KEY_REJECTED,
                                             PORT_FEEDBACK_TYPE_KEY_REJECTED,
                                         },
                                         3,
                                        ...
                                        {"*#*",
                                         {
                                             PORT_FEEDBACK_TYPE_KEY_ACCEPTED,
                                             PORT_FEEDBACK_TYPE_MESSAGE_INVALID,
                                             PORT_FEEDBACK_TYPE_KEY_ACCEPTED,
                                         },
                                         3,
                                         {nexus_keycode_frame_filled("")},
    ...
    void)
{
    ...
    for (uint8_t i = 0; i < sizeof(scenarios) / sizeof(scenarios[0]); ++i)
    {
        ...
        for (uint16_t j = 0; scenario.input_chars[j] != '\0'; ++j)
        {
            ...
            nx_keycode_process(nexus_keycode_core_uptime());
    ...
    void)
{
    ...
    nx_keycode_process(0); // complete internal init

    port_feedback_start_ExpectAndReturn(PORT_FEEDBACK_TYPE_KEY_ACCEPTED, true);
