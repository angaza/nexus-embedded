...
NEXUS_STATIC_ASSERT(
    sizeof(_this_core.stored) ==
        (NX_NV_BLOCK_0_LENGTH - NEXUS_NV_BLOCK_ID_WIDTH -
...
NEXUS_IMPL_STATIC void nexus_keycode_mas_finish(void)
{
    ...
    else
    {
        ...
        port_feedback_start(PORT_FEEDBACK_TYPE_MESSAGE_INVALID);
    }
...
NEXUS_IMPL_STATIC uint32_t nexus_keycode_mas_bookend_process(void)
{
    ...
    if (_this_bookend.start_seen)
    {
        ...
        {
            _this_bookend.latest_uptime = nexus_keycode_core_uptime();
        }

        // check for message-receipt timeout
        uint32_t elapsed =
            nexus_keycode_core_uptime() - _this_bookend.latest_uptime;
...
NEXUS_IMPL_STATIC void nexus_keycode_mas_bookend_push(const nx_keycode_key key)
{
    ...
    {
        port_feedback_start(PORT_FEEDBACK_TYPE_KEY_REJECTED);
    }
    else if (key == _this_bookend.start)
    {
        port_feedback_start(PORT_FEEDBACK_TYPE_KEY_ACCEPTED);
...
#if NEXUS_KEYCODE_END_CHAR == NEXUS_KEYCODE_UNDEFINED_END_CHAR
