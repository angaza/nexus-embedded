...
const uint8_t NEXUS_KEYCODE_MESSAGE_LENGTH_MAX_DIGITS_SMALL = 14;
...
NEXUS_STATIC_ASSERT(
    sizeof(_nexus_keycode_stored) ==
        (NX_NV_BLOCK_1_LENGTH - NEXUS_NV_BLOCK_ID_WIDTH -
...
uint32_t nexus_keycode_pro_process(void)
{
    ...
    if (!_this_core.pending)
    {
        return NEXUS_KEYCODE_IDLE_TIME_BETWEEN_PROCESS_CALLS_SECONDS;
    }
    ...
    enum port_feedback_type feedback = PORT_FEEDBACK_TYPE_NONE;

    switch (response)
    {
        case NEXUS_KEYCODE_PRO_RESPONSE_INVALID:
            feedback = PORT_FEEDBACK_TYPE_MESSAGE_INVALID;

            break;

        case NEXUS_KEYCODE_PRO_RESPONSE_VALID_DUPLICATE:
            feedback = PORT_FEEDBACK_TYPE_MESSAGE_VALID;

            break;

        case NEXUS_KEYCODE_PRO_RESPONSE_VALID_APPLIED:
            feedback = PORT_FEEDBACK_TYPE_MESSAGE_APPLIED;

            break;

        case NEXUS_KEYCODE_PRO_RESPONSE_DISPLAY_DEVICE_ID:
            feedback = PORT_FEEDBACK_TYPE_DISPLAY_SERIAL_ID;

            break;

        case NEXUS_KEYCODE_PRO_RESPONSE_NONE:
            feedback = PORT_FEEDBACK_TYPE_NONE;
    ...
    (void) port_feedback_start(feedback);
    ...
    const struct nexus_keycode_pro_small_message* message)
{
    ...
    const struct nx_check_key secret_key = port_identity_get_secret_key();
        ...
        (uint8_t) NEXUS_KEYCODE_PRO_SMALL_MAINTENANCE_OR_TEST_TYPE)
    {
            ...
            else
            {
                const uint16_t increment_days =
                    nexus_keycode_pro_small_get_set_credit_increment_days(
                        message->body.activation.increment_id);
                port_payg_credit_set(increment_days *
                 ...
                 (uint8_t) NEXUS_KEYCODE_PRO_SMALL_ACTIVATION_ADD_CREDIT_TYPE)
        {
            ...
            if (port_payg_state_get_current() != PAYG_STATE_UNLOCKED)
            {
                ...
                {
                    port_payg_credit_add(increment_days *
                                         NEXUS_KEYCODE_PRO_SECONDS_IN_DAY);
                }
            }
            else
    ...
    else
    {
        ...
        switch (message->body.maintenance_test.function_id)
        {
            ...
            case NEXUS_KEYCODE_PRO_SMALL_ENABLE_SHORT_TEST:
                if (port_payg_state_get_current() == PAYG_STATE_DISABLED)
                {
                    test_applied = true;
                    test_credit_secs =
            ...
            case NEXUS_KEYCODE_PRO_SMALL_ENABLE_QC_TEST:
                if (nexus_keycode_pro_get_long_qc_code_count() <
                    ...
                    port_payg_state_get_current() != PAYG_STATE_UNLOCKED)
                {
                    test_applied = true;
                    test_credit_secs =
        ...
        if (test_applied)
        {
            port_payg_credit_add(test_credit_secs);
    ...
    struct nexus_keycode_pro_full_message* parsed)
{
             ...
             (uint8_t) NEXUS_KEYCODE_PRO_FULL_PASSTHROUGH_COMMAND)
    {
            ...
            digits.length < NEXUS_KEYCODE_MESSAGE_LENGTH_MAX_DIGITS_FULL)
        {
            ...
            const enum port_passthrough_error result =
                port_passthrough_keycode(&passthrough_code);

            if (result == PORT_PASSTHROUGH_ERROR_NONE)
            {
                command_valid = true;
            }
        }
    ...
    const struct nexus_keycode_pro_full_message* message)
{
    ...
    const struct nx_check_key secret_key =
        message->type_code <
                (uint8_t) NEXUS_KEYCODE_PRO_FULL_FACTORY_ALLOW_TEST ?
            port_identity_get_secret_key() :
...
// certain version of clang misinterpret this section
    ...
    switch (message->type_code)
    {
        case NEXUS_KEYCODE_PRO_FULL_ACTIVATION_ADD_CREDIT:
            ...
            if (port_payg_state_get_current() != PAYG_STATE_UNLOCKED)
            {
                port_payg_credit_add(credit_increment_seconds);
            }
            else
            {
        ...
        case NEXUS_KEYCODE_PRO_FULL_ACTIVATION_SET_CREDIT:
            ...
            {
                port_payg_credit_set(credit_increment_seconds);
            }
            break;
    ...
    const struct nexus_keycode_pro_full_message* message)
{
    ...
    switch (message->type_code)
    {
        ...
        case NEXUS_KEYCODE_PRO_FULL_FACTORY_QC_TEST:
        {
            ...
            if (test_applied)
            {
                port_payg_credit_add(qc_credit_seconds);
...
nexus_keycode_pro_can_unit_accept_qc_code(const uint32_t qc_credit_seconds)
{
    ...
    const enum payg_state payg_state_before = port_payg_state_get_current();

    if (payg_state_before == PAYG_STATE_UNLOCKED)
    {
        return false;
    }
    ...
    if (qc_credit_seconds != NEXUS_KEYCODE_PRO_QC_LONG_TEST_MESSAGE_SECONDS &&
        payg_state_before != PAYG_STATE_DISABLED)
    {
        return false;
    }
