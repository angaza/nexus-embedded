...
struct nx_nv_block_meta NX_NV_BLOCK_KEYCODE_MAS = {
    .block_id = 0, .length = NX_NV_BLOCK_0_LENGTH};
struct nx_nv_block_meta NX_NV_BLOCK_KEYCODE_PRO = {
    .block_id = 1, .length = NX_NV_BLOCK_1_LENGTH};

// Used internally to compute CRC given a pointer to start of a full block
uint16_t nexus_nv_compute_crc(const struct nx_nv_block_meta block_meta,
...
bool nexus_nv_update(const struct nx_nv_block_meta block_meta,
                     uint8_t* inner_data)
{
    // read existing block from NV
    uint8_t existing_block[NX_NV_MAX_BLOCK_LENGTH] = {0};
    if (port_nv_read(block_meta, existing_block))
    {
    ...
    NEXUS_ASSERT(inner_data_size < NX_NV_MAX_BLOCK_LENGTH,
                 "Invalid new block inner data size!");

    // compute a new valid "NX NV Block"
    uint8_t new_block[NX_NV_MAX_BLOCK_LENGTH] = {0};
    ...
    return port_nv_write(block_meta, new_block);
}

bool nexus_nv_read(const struct nx_nv_block_meta block_meta,
                   uint8_t* inner_data)
{
    uint8_t block[NX_NV_MAX_BLOCK_LENGTH] = {0};

    port_nv_read(block_meta, block);
    if (nx_nv_block_valid(block_meta, block))
    {
...
NEXUS_STATIC_ASSERT(
    NEXUS_NV_BLOCK_ID_WIDTH == sizeof(((struct nx_nv_block_meta*) 0)->block_id),
