/** \file
 * Nexus Keycode Core Module (Header)
 * \author Angaza
 * \copyright 2020 Angaza, Inc.
 * \license This file is released under the MIT license
 *
 * The above copyright notice and license shall be included in all copies
 * or substantial portions of the Software.
 */

#ifndef __NEXUS__KEYCODE__SRC__NEXUS_KEYCODE_CORE_H_
#define __NEXUS__KEYCODE__SRC__NEXUS_KEYCODE_CORE_H_

#include "src/internal_keycode_config.h"
#include "src/nexus_keycode_pro.h"

#include <stdbool.h>
#include <stdint.h>

// Protocol-specific (small or full) initialization parameters
struct nexus_keycode_handling_config
{
    nexus_keycode_pro_parse_and_apply parse_and_apply;
    nexus_keycode_pro_protocol_init keycode_protocol_init;
    uint8_t stop_length;
    nx_keycode_key start_char;
    nx_keycode_key end_char;
    const nx_keycode_key* keycode_alphabet;
};

/** Seconds between process calls when idle.
 *
 * If nothing requires processing any sooner, `nx_keycode_process` will
 * return this value.
 */
extern const uint32_t NEXUS_KEYCODE_IDLE_TIME_BETWEEN_PROCESS_CALLS_SECONDS;

/** Has the Nexus Keycode subsystem successfully initialized?
 *
 * Initialization is marked 'successful' once:
 *
 * 1) System calls `nx_keycode_init`
 * 2) System subsequently calls `nx_keycode_process`
 *
 * \return True if initialized successfully, false otherwise
 */
bool nexus_keycode_core_init_completed(void);

/** Number of seconds
 *
 * This is only updated when `nx_keycode_process` is run, and should be
 * not be considered 'up to date' when examined in an interrupt as a result.
 */
uint32_t nexus_keycode_core_uptime(void);

/** Internal functions, not for calls outside this module.
 *
 * The `NEXUS_INTERNAL_IMPL_NON_STATIC` flag exposes these functions in
 * the header if in a unit testing scenario.
 */
#ifdef NEXUS_INTERNAL_IMPL_NON_STATIC
void nexus_keycode_core_internal_init(
    const struct nexus_keycode_handling_config* config);
#endif

#endif
